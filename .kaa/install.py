...
def _install_pykaa() -> None:
    ...
    uri = (
        "https://%s@artifactory.globaldevtools.bbva.com/artifactory/api/pypi/gl-datio-runtime-pypi-local/simple/"
        % (credentials)
    )
    command = "pip install %s pykaa==0.6.0 pykaa-dataproc-aws -U --extra-index-url %s" % (user_flag, uri)
